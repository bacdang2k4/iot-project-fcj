...
// Ngưỡng và Giá trị
#define BASE_READING      571  // Giá trị môi trường sạch
#define ALCOHOL_THRESHOLD 0.05 // Ngưỡng cảnh báo cồn
...
void loop() {
  ...
  if (WiFi.status() == WL_CONNECTED) {
      if (!client.connected()) {
         // Reconnect logic giản lược (như code gốc)
      }
      client.loop(); 
  }
    ...
    case ST_CHECK_HEALTH:
      {
        ...
        if (violation) {
          ...
          // --- Thêm hiệu ứng còi + đèn ---
          digitalWrite(LED_PIN, HIGH);
          tone(BUZZER_PIN, 2000);   // kêu 2kHz
          delay(2000);
          noTone(BUZZER_PIN);
          digitalWrite(LED_PIN, LOW);
